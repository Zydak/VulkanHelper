...
namespace VulkanHelper
{
    ...
    *              - [2] Normals.
    */
    void Denoiser::ImageToBuffer(VkCommandBuffer cmdBuf, const std::vector<VulkanHelper::Image*>& imgIn)
    {
    ...
    */
    void Denoiser::BufferToImage(VkCommandBuffer cmdBuf, VulkanHelper::Image* imgOut)
    {
        VkBufferImageCopy region = {
            .imageSubresource = {.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT, .layerCount = 1},
            .imageExtent = {.width = m_ImageSize.width, .height = m_ImageSize.height, .depth = 1},
