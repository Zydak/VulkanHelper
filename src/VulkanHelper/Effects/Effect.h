...
namespace VulkanHelper
{
	...
	private:
		...
		std::vector<VulkanHelper::DescriptorSetLayout::Binding> m_Bindings;
	...
	template<typename T>
	Effect<T>& VulkanHelper::Effect<T>::operator=(Effect<T>&& other) noexcept
	{
		if (m_Initialized)
			Destroy();
	...
	template<typename T>
	VulkanHelper::Effect<T>::Effect(Effect&& other) noexcept
	{
		if (m_Initialized)
			Destroy();
	...
	template<typename T>
	VulkanHelper::Effect<T>::Effect(const CreateInfo& createInfo)
	{
		Init(createInfo);
	}

	template<typename T>
	void VulkanHelper::Effect<T>::Reset()
	{
		m_ShaderPath.clear();
	...
	template<typename T>
	VulkanHelper::Effect<T>::~Effect()
	{
		Destroy();
	}

	template<typename T>
	void VulkanHelper::Effect<T>::Run(VkCommandBuffer cmd)
	{
		...
		for (int i = 0; i < m_AdditionalImages.size(); i++)
		{
			if (m_AdditionalImages[i]->GetLayout() != VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL)
				m_AdditionalImages[i]->TransitionImageLayout(VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VulkanHelper::Renderer::GetCurrentCommandBuffer());
		}
	...
	template<typename T>
	void VulkanHelper::Effect<T>::RecompileShader(std::vector<Shader::Define> defines)
	{
	...
	template<typename T>
	void VulkanHelper::Effect<T>::Destroy()
	{
		if (!m_Initialized)
			return;
	...
	template<typename T>
	void VulkanHelper::Effect<T>::Init(const CreateInfo& createInfo)
	{
		if (m_Initialized)
			Destroy();
		...
		m_Bindings.clear();
		{
			...
			m_Descriptor.Init(&VulkanHelper::Renderer::GetDescriptorPool(), m_Bindings);
